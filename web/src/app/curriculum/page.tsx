  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-6 sm:py-8">
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">📚 教材同步</h1>
      ...
      <div className="mb-4 rounded-2xl border border-gray-200 bg-white p-3">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-1">
            <span className="text-xs text-gray-500">学期</span>
            <select
              value={semester}
      ...
      {weekly && (
        <section className="mb-6 rounded-2xl border border-gray-200 bg-white p-5">
          ...
          {weekly.chapters.length === 0 ? (
            <p className="text-sm text-gray-400">该学期暂无章节数据</p>
          ) : (
            <ul className="space-y-2">
              {weekly.chapters.map((c, i) => (
                <li
                  key={i}
                  className="flex items-start justify-between rounded-lg border border-gray-100 p-3 hover:border-blue-200 hover:bg-blue-50/30"
                >
                  <div className="flex-1">
                    <p className="font-medium text-gray-800">{c.chapter}</p>
      ...
      <section className="rounded-2xl border border-gray-200 bg-white p-5">
        <div className="mb-3 flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold text-gray-800">2024-2026 改版追踪</h2>
          <span className="text-xs text-gray-400">({changes.length} 条)</span>
        </div>

        {loading ? (
          <p className="text-sm text-gray-400">加载中…</p>
        ) : changes.length === 0 ? (
          <p className="text-sm text-gray-400">该学科/年级暂无改版信息</p>
        ) : (
            ...
            {changes.map((c) => (
              ...
              >
                <div className="mb-2 flex flex-wrap items-center gap-2 text-xs">
                  ...
                  {c.verified ? (
                    <span className="inline-flex items-center gap-0.5 rounded bg-emerald-100 px-2 py-0.5 text-emerald-700">
                      <CheckCircle2 className="h-3 w-3" />
                      已确认
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-0.5 rounded bg-amber-100 px-2 py-0.5 text-amber-700">
                      <AlertCircle className="h-3 w-3" />
                      待复核
                    </span>
                  )}
                  <span className="text-gray-400">{c.change_count} 项改动</span>
                </div>
...
function ChangeDetail({ changeId }: { changeId: number }) {
  ...
  if (!detail) {
    return <p className="text-xs text-gray-400">加载…</p>;
  }

  return (
    <div className="space-y-2">
      {detail.changes.map((ch, i) => {
        const badge = CHANGE_TYPE_BADGE[ch.type] || {
          label: ch.type,
        ...
        return (
          <div key={i} className="rounded border border-gray-100 bg-gray-50/50 p-2">
            <div className="mb-1 flex items-center gap-2 text-xs">
              <span className={`rounded px-1.5 py-0.5 ${badge.color}`}>
                {badge.label}
              </span>
              <span className="font-medium text-gray-700">{ch.chapter}</span>
            </div>
